...
iyr:2010 hgt:158cm hcl:#b6652a ecl:blu byr:1944 eyr:2021 pid:42
"""

puzzle = """byr:2010 pid:#1bb4d8 eyr:2021 hgt:186cm iyr:2020 ecl:grt
...
if TEST:
    puzzle = test_puzzle
puzzle = puzzle.split("\n\n")
puzzle = [x.replace("\n", " ").split(" ") for x in puzzle]


def is_valid(passport):
    """ require fields are in passport key values """
    REQUIRED = ["byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid"]
    # convert passport k/v pairs to dict
    passport_dict = dict(
        zip(
            [x.split(":")[0] for x in passport if len(x.strip()) > 0],
            [x.split(":")[1] for x in passport if len(x.strip()) > 0],
        )
    )
    ...
    for r in REQUIRED:
        assert (
            r in passport_dict.keys()
        ), f"{r} field is missing"

    # byr
    assert (
        int(passport_dict["byr"]) >= 1920 and int(passport_dict["byr"]) <= 2002
    ), f"byr check failed {passport_dict['byr']} not between 1920/2002"

    # iyr
    assert (
        int(passport_dict["iyr"]) >= 2010 and int(passport_dict["iyr"]) <= 2020        
    ), f"iyr check failed {passport_dict['iyr']} not between 2010/2020"

    # eyr
    assert (
        int(passport_dict["eyr"]) >= 2020 and int(passport_dict["eyr"]) <= 2030        
    ), f"eyr check failed {passport_dict['iyr']} not between 2020/2030"

    # hgt
    m = re.search("^(?P<hgt>\d*)(?P<unit>(cm|in))$", passport_dict["hgt"])
    assert(
        (m["unit"] == "cm" and int(m["hgt"]) >= 150 and int(m["hgt"]) <= 193)
        or
        (m["unit"] == "in" and int(m["hgt"]) >= 59 and int(m["hgt"]) <= 76)
    ), f"hgt check failed on {passport_dict['hgt']}"

    # hcl
    m = re.search("^(?P<hcl>#[0-9a-f]{6})$", passport_dict["hcl"])
    assert(
        m is not None
    ), f"hcl check failed on {passport_dict['hcl']}"

    # ecl
    assert(
        passport_dict["ecl"] in ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"]
    ), f"ecl check failed on {passport_dict['ecl']}"

    # pid
    m = re.search("^(?P<pid>\d{9})$", passport_dict["pid"])
    assert(
        m is not None
    ), f"pid check failed on {passport_dict['pid']}"
