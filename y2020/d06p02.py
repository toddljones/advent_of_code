...
# preprocess the puzzle, convert to list of lists
puzzle = puzzle.split("\n\n")
puzzle = [x.splitlines() for x in puzzle]
...
for group in puzzle:
    ...
    flat_group = []
    for i in group:
        flat_group.extend(set(i))
    for response, response_cnt in dict(Counter(flat_group)).items():
        if response_cnt == group_len:
            all_yes_cnt += 1
print(all_yes_cnt)
