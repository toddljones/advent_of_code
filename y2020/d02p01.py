test_puzzle = """1-3 a: abcde
1-3 b: cdefg
2-9 c: ccccccccc""".splitlines()

puzzle = """6-10 s: snkscgszxsssscss
6-7 b: bbbbbxkb
2-4 n: nnnjn
1-2 j: jjjj
5-9 z: jgzzzqhbj
...
valid_cnt = 0
for line in puzzle:
    count_lbound = int(line.split(" ")[0].split("-")[0])
    count_ubound = int(line.split(" ")[0].split("-")[1])
    letter = line.split(" ")[1].strip(":")
    password = line.split(" ")[2]

    occurrences = password.count(letter)

    if occurrences >= count_lbound and occurrences <= count_ubound:
        valid_cnt += 1
