test_puzzle = """ecl:gry pid:860033327 eyr:2020 hcl:#fffffd
...
def is_valid(passport):
    ...
    passport_fields = [x.split(":")[0] for x in passport]
    for r in REQUIRED:
        if r not in passport_fields:
            print(f"{r} is missing from {passport}")
            return False
        if 
    return True
